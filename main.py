...
from plotters import plot_icegraph, extract_station_info, plot_icing_map, create_station_selector
...
# def main():
#     """
#     Streamlit UI for visualizing icing data on a map.
#     Allows user to select stations and time range, fetch data from FMI, and display graphs below the map.
#     """
#     st.set_page_config(page_title="Icing Map And Graph", layout="centered")
#     st.title("Icing On Map")

#     for key in ["show_map", "station_data", "figure_data", "selected_station"]:
...
#     sorted_places = dict(sorted(places.items()))
...
#                         if s["name"] == selected_station), None)
#             if idx is not None:
#                 fig = st.session_state.figure_data[idx]
#                 buf = BytesIO()
#                 fig.savefig(buf, format="png")
#                 buf.seek(0)
#                 st.image(buf, caption=f"Jäätämisdata: {selected_station}", width='stretch')
